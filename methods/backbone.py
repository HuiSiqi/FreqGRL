...
class SimpleBlock(nn.Module):
  maml = False
  def __init__(self, indim, outdim, half_res, leaky=False,se_layer=False,**kwargs):
...
class ResNet(nn.Module):
  maml = False
  def __init__(self,block,list_of_num_layers, list_of_out_dims, flatten=True, leakyrelu=False,se_layers=[],low_freq_layers=[],**kwargs):
    ...
    for i in range(4):
      for j in range(list_of_num_layers[i]):
        ...
        B = block(indim, list_of_out_dims[i], half_res, leaky=leakyrelu,se_layer=i in se_layers, low_freq=i in low_freq_layers)
        trunk.append(B)
...
if __name__ == '__main__':
    ...
    print('------------------')
    model_func = model_dict['ResNet10']
    net = model_func(flatten = False, leakyrelu= False)
    from torch.autograd import Variable
    x = Variable(torch.randn([16,3,224,224]))
    out = net(x)
    print(out.size())

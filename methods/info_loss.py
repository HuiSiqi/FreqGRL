...
def gcsd_QMI(*variants: Union[list,tuple,Tensor], 
        sigmas: Union[Tensor,ndarray,float]=None,
        **kwargs) -> Tensor:
    Res = [calculate_gram(variant) for variant in variants]
    G = torch.stack([gram for gram,_ in Res])
    if sigmas is None:
        sigmas = torch.stack([sigma for _,sigma in Res])
    P_joint=G.prod(0).mean(-1)
    P_views=G.mean(-1)

    cross = -2.0*atleast_epsilon((P_joint*P_views.prod(0)).mean()).log()
    power_joint  = atleast_epsilon(P_joint.pow(2.0).mean()).log()
    power_product= atleast_epsilon(P_views.pow(2.0).prod(0).mean()).log()

...
@torch.no_grad()
def swap_low_freq_mutual(S, A, name):
    """
    ...
    # 2. 构建低频掩码
    if name=='fix_lamb_0.1':
        radius =  int(0.1*224)
    elif name=='fix_lamb_0.2':
        radius =  int(0.2*224)
    elif name=='random_lamb_0_1':
        radius = np.random.beta(1.0, 1.0) * 224
    else:
        radius =  np.random.beta(1.0,1.0) * 40
...
class GnnNetStudent(MetaTemplate):
    ...
    def train_loop(self, epoch, S_train_loader, A_train_loader, optimizer, total_it):
        ...
        for ((i, (S_x, S_y_global)), (i, (A_x, A_y_global))) in zip(enumerate(S_train_loader), enumerate(A_train_loader)):
            ...
            if self.args.FM:
                S_x, A_x = swap_low_freq_mutual(S_x, A_x,name=self.args.ablate_exp)
                S_scores2, S_loss_fsl2, S_loss_cls2 = self.forward_loss(S_x, S_y_global, data_flag='S')
                A_scores2, A_loss_fsl2, A_loss_cls2 = self.forward_loss(A_x, A_y_global, data_flag='A')
                S_loss2 = S_loss_fsl2
                A_loss2 = A_loss_fsl2
            else:
                S_loss2 = A_loss2 = 0

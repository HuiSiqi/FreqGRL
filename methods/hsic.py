...
def dHSIC(*argv,sigma=None):
    ...
    K_list = [gaussian_grammat(_arg,sigma=sigma) for _arg in argv]

    return dHSIC_calc(K_list)

if __name__ == '__main__':
    x = 10*numpy.random.randn(85)
    y = x + 0.1 * numpy.random.randn(85)
    z = x + 0.1 * numpy.random.randn(85)
    hsic = dHSIC(x,y,z,sigma=None)
    print(hsic)
    hsic = dHSIC(x,y,z,sigma=0.1)
    print(hsic)
    hsic = dHSIC(x,y,z,sigma=0.8)
    print(hsic)
    hsic = dHSIC(x,y,z,sigma=1.0)
    print(hsic)
    hsic = dHSIC(x,y,z,sigma=10)
    print(hsic)
    hsic = dHSIC(x,y,z,sigma=100)
    print(hsic)
    hsic = dHSIC(x,y,z,sigma=1000)
    print(hsic)
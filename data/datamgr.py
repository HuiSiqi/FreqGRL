...
from data.dataset import SimpleDataset, SetDataset, MultiSetDataset, EpisodicBatchSampler, WeightedEpisodicBatchSampler, MultiEpisodicBatchSampler, RandomLabeledTargetDataset
...
class SimpleDataManager(DataManager):
  ...
  def get_data_loader(self, data_file, aug, data_loader_params = dict(shuffle = True, num_workers = 4, pin_memory = True)): #parameters that would change on train/val set
...
class SetDataManager(DataManager):
  def __init__(self, image_size, n_way, n_support, n_query, n_eposide=100, strong_aug=False):
  ...
  def get_data_loader(self, data_file, aug, data_loader_params = dict(num_workers=4, pin_memory=True),return_gt=False,weighted_sampler=False): #parameters that would change on train/val set
    transform = self.trans_loader.get_composed_transform(aug)
